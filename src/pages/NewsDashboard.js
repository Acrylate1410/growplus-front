...
export function NewsDashboard() {
    ...
    useEffect(() => {
      fetch("https://okyibhzr7o.genhosting.net/growplus/articles/get_articles").then(res => res.json()).then(data => {
        setArticleList(data || [])
      }).catch(error => {})
    }, []);
    ...
    const handleDelete = () => {
        fetch("https://okyibhzr7o.genhosting.net/growplus/articles/delete", {
            method: "DELETE",
        ...
        <>
            ...
            <header className='header p-4 bg-[#9ec7a5] flex items-center justify-between fixed top-0 right-0 left-0 z-20'>
                ...
                <div className="flex items-center">
                    <input className="p-2 mr-8 w-96 rounded-lg outline-0" placeholder="Tìm bài viết theo tiêu đề" onInput={(e) => setFilter(e.target.value)}></input>
                    <Link to="/add_article">
                        <div className="bg-[#3b8b59] text-white w-48 h-12 rounded-full flex items-center justify-center border border-[#3b8b59] hover:bg-[#9ec7a5] hover:text-[#3b8b59] transition">
                            <AiOutlineFileAdd className="text-xl"/>
                            <p className="ml-2">Đăng bài viết mới</p>
                        </div>
                    </Link>
                    <div className="bg-[#3b8b59] text-white w-40 h-12 rounded-full flex items-center justify-center border border-[#3b8b59] hover:bg-[#9ec7a5] hover:text-[#3b8b59] transition ml-4 cursor-pointer" onClick={() => setModalOn("fixed top-0 bottom-0 right-0 left-0 z-30")} >
                        <BsTrash className="text-xl"/>
                        <p className="ml-2">Xóa {selected.length !== 0 && selected.length} bài viết</p>
                    </div>
                </div>
            </header>
            <table className="table my-4 px-4 w-max md:w-full mt-24">
                <tbody>
                    <tr>
