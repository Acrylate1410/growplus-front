...
export function ArticleUploadForm() {
    ...
    useEffect(() => {
        if (location.pathname.endsWith("edit_article")) {
            fetch("https://okyibhzr7o.genhosting.net/growplus/articles/get_one_article/" + params.get("id")).then(res => res.json()).then(data => {
    ...
    const handleSubmit = (e) => {
        ...
        if (isThumbnailLegal && isContentLegal) {
            ...
            if (location.pathname.endsWith("add_article")) {
                ...
                fetch("https://okyibhzr7o.genhosting.net/growplus/articles/update/" + params.get("id"), {
                    method: "PATCH",
                    body: formData
                }).then(alert("Đã đăng bài thành công")).catch(error => {})
            }
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit} className="h-[100vh] w-full flex flex-col justify-center items-center">
                ...
                <div className="my-4"></div>
                <textarea required ref={description} rows="3" className="outline-0 border border-black w-4/5 md:w-3/5 p-2"  placeholder="Mô tả bài viết" defaultValue={articleToEdit.description}></textarea>
                <div className="my-4"></div>
                ...
                <div className="flex items-center">
                    <h2>File bài viết:</h2>
                    <div className="mx-2"></div>
                    <input required type="file" onChange={(e) => setContent(e.target.files[0])}></input>
                </div>
                <div className="my-4"></div>
