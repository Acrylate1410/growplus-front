...
export function ArticleUploadForm() {
    ...
    useEffect(() => {
        if (location.pathname.endsWith("edit_article")) {
            fetch("https://api.kenkojapan.vn/growplus/articles/get_one_article/" + params.get("id")).then(res => res.json()).then(data => {
    ...
    const handleSubmit = (e) => {
        ...
        if (isThumbnailLegal && isContentLegal) {
            ...
            if (location.pathname.endsWith("add_article")) {
                ...
                fetch("https://api.kenkojapan.vn/growplus/articles/update/" + params.get("id"), {
                    method: "PATCH",
                    body: formData
                }).then(alert("Đã đăng bài thành công")).catch(error => {})
            }
